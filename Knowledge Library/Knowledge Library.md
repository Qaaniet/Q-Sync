...
```dataviewjs
const categories = ["Science", "History", "Psychology", "Language"];
...
function render(category) {
    ...
    if (category === "All") {
        categories.forEach(cat => {
            dv.header(2, cat);
            let pages = dv.pages(`#hub/knowledge and #category/${cat.toLowerCase()}`).sort(p => p.file.ctime, 'desc');
            if (pages.length === 0) {
        ...
        let pages = dv.pages(`#hub/knowledge and #category/${category.toLowerCase()}`).sort(p => p.file.ctime, 'desc');
        if (pages.length === 0) {
            dv.paragraph("_No notes yet in this category._");
        } else {
            dv.table(["Title", "Created", "Modified"],
